...
- **Media Library Integration Hardening & Configurable Toggle**: Added option `tsbifw_enable_media_column` (default enabled) allowing store owners to toggle the Media Library list view status column and filter dropdown on or off. Hardened `filter_media_by_indexed_status()` to safely merge `post__in` and `post__not_in` without wiping concurrent filters, and introduced defensive filter ID capping via `tsbifw_max_media_filter_ids` (default 5,000) to protect database servers from packet exhaustion on massive stores.
