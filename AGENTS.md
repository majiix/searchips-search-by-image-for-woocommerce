...
- Document External Services: If the plugin connects to or relies on any third-party or external services (including APIs like OpenRouter), this must be fully disclosed in the `readme.txt` file under an `== External services ==` section. You must explicitly document: what data is sent, when/why it is sent, and provide valid, direct links to that service's Terms of Service and Privacy Policy.
- Scripts and Styles Enqueuing: Always use official WordPress functions and hooks to include static and dynamic scripts or stylesheets. Utilize `wp_enqueue_script()` and `wp_enqueue_style()` hooked to `wp_enqueue_scripts` for public pages, or `admin_enqueue_scripts` for backend administration panels. Never output scripts or styles directly in page layouts.
- Bundled Libraries Maintenance: Ensure any third-party libraries (such as JavaScript utilities or helper scripts) packaged within the plugin are updated to their latest stable version releases to maintain platform safety and compatibility.
